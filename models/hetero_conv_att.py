...
class HeagNetConvAtt(torch.nn.Module):
    def __init__(
        self,
        meta: Metadata,
        ...
        **kwargs,
        ...
        # attention component for gathering all msg from different edge types
        attn_weights = {}
        for et in self.edge_types:
            src, rel, dst = et
            str_edge_type = "__".join(et)
            attn_weights[str_edge_type] = torch.nn.Parameter(
                torch.randn(out_channels_node[dst]), requires_grad=True
            )

            if bidirectional and src != dst:
                rev_str_edge_type = "__".join([dst, "rev_" + rel, src])
                attn_weights[rev_str_edge_type] = torch.nn.Parameter(
                    torch.randn(out_channels_node[src]), requires_grad=True
                )
        ...
        if self.node_self_loop:
            ...
            for nt in self.node_types:
                node_loop[nt] = Linear(
                    in_channels_node[nt], out_channels_node[nt], bias=bias
                )

            self.node_loop = nn.ModuleDict(node_loop)
    ...
    def forward(
        ...
        a_out_list_dict = defaultdict(list)
        ...
        for et, adj in adj_dict.items():
            ...
            # att
            alpha = out @ self.attn_weights[str_edge_type]
            a_out_list_dict[dst].append(alpha)
            ...
            if self.bidirectional and src != dst:
                ...
                # att
                alpha = out @ self.attn_weights[rev_str_edge_type]
                a_out_list_dict[src].append(alpha)
        ...
        for nt in x_dict.keys():
            if len(a_out_list_dict[nt]) > 0:
                alphas = torch.stack(a_out_list_dict[nt], dim=1)
                alphas = torch.softmax(alphas, dim=1)

                out = torch.stack(x_out_list_dict[nt], dim=-1)
                out = torch.bmm(out, alphas[:, :, None]).squeeze()

                if self.node_self_loop:
                    out += self.node_loop[nt](x_dict[nt])
            else:
                if self.node_self_loop:
                    out = self.node_loop[nt](x_dict[nt])
                else:
