...
from models.net import HeagNet
...
parser.add_argument("--n-epoch", type=int, default=1, help="number of epoch")
...
# load datasets
storage = torch.load("storage/telecom-large-anomaly.pt")
...
num_neighbors_dict = {
    ...
    ("user", "live", "cell"): 5,
    ("user", "use", "app"): 5,
    ("package", "rev_buy", "user"): 5,
    ("cell", "rev_live", "user"): 5,
    ("app", "rev_use", "user"): 5,
}
...
dataloader = MultiNeighborLoader(
    ...
    # batch_size=[512, 128, 256, 128],
    # batch_size=[256, 256, 256, 256],
    # batch_size=[32, 32, 32, 32],
    # input_repeat=[1.0, 1.0, 1.0, 1.0],
    # input_repeat=[0.8, 0.8, 0.4, 0.8],
    input_repeat=[0.05, 0.8, 0.05, 0.8],
    drop_last=True,
)
...
def eval():
    ...
    # inference
    node_batch_size = {key: 2**13 if key in ["cell"] else 2**12 for key in ntypes}
    mlp_batch_size = {key: 2**13 for key in ntypes}
    epred_batch_size = {key: 2**13 for key in etypes}
    ...
    with torch.no_grad():
        ...
        eval_metrics_mean = compute_evaluation_metrics(dataset, score, agg="mean")
    ...
    print(
        f"      --> Metric mean: "
        + f"node-auc-roc: {eval_metrics_mean['node_avg_roc_auc']:.4f}, edge-auc-roc: {eval_metrics_mean['edge_avg_roc_auc']:.4f}, "
        + f"node-auc-pr {eval_metrics_mean['node_avg_pr_auc']:.4f}, edge-auc-pr {eval_metrics_mean['edge_avg_pr_auc']:.4f} "
        + f"| {time.time()-tz:.4f}s",
    )

    ## each node / edge
    for nt in ntypes:
        print(f"{nt} x_loss: {loss_component['x_loss_dict'][nt].mean()}")
    for et in etypes:
        print(
            f"{et}: xe_loss: {loss_component['xe_loss_dict'][et].mean()}, structure_loss: {loss_component['structure_loss_dict'][et].mean()}"
        )
    ...
    for nt in ntypes:
        print(
            f"{nt} (mean): roc auc: {eval_metrics_mean['node_result_dict'][nt]['roc_auc']}, pr auc: {eval_metrics_mean['node_result_dict'][nt]['pr_auc']}"
        )
    for et in etypes:
        print(
    ...
    output_stored = {
        ...
        "metrics_mean": eval_metrics_mean,
        "node_score_agg": node_score_agg,
    }
    torch.save(
        model_stored,
        os.path.join(result_dir, f"graphbean1-{args['name']}-{args['id']}-model.th"),
    )
    torch.save(
        output_stored,
        os.path.join(result_dir, f"graphbean1-{args['name']}-{args['id']}-output.th"),
    )
...
# eval()
print(f">> graphbean1-{args['name']}-{args['id']} >> DONE >>")
