...
parser.add_argument("--n-epoch", type=int, default=30, help="number of epoch")
...
def eval():
    ...
    torch.save(
        model_stored,
        os.path.join(result_dir, f"graphbean-{args['name']}-{args['id']}-model.th"),
    )
    torch.save(
        output_stored,
        os.path.join(result_dir, f"graphbean-{args['name']}-{args['id']}-output.th"),
    )
...
# eval()
print(f">> graphbean-{args['name']}-{args['id']} >> DONE >>")
